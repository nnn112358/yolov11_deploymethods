    ...
    for index in range(head_num):
        ...
        for h in range(map_size[index][0]):
            for w in range(map_size[index][1]):
                ...
                cls_index = 0
                cls_vlaue = -1
                for c in range(64, class_num + 64, 1):
                    val = output[c * map_size[index][0] * map_size[index][1] + h * map_size[index][1] + w]
                    if val > cls_vlaue:
                        cls_vlaue = val
                        cls_index = c - 64
                ...
                if cls_vlaue > object_thresh:
                    ...
                    for lc in range(4):
                        ...
                        locval = 0
                        for df in range(16):
                            temp = exp(output[((lc * 16) + df) * map_size[index][0] * map_size[index][1] + h * map_size[index][1] + w])
                            output[((lc * 16) + df) * map_size[index][0] * map_size[index][1] + h * map_size[index][1] + w] = temp
                            sfsum += temp

                        for df in range(16):
                            sfval = output[((lc * 16) + df) * map_size[index][0] * map_size[index][1] + h * map_size[index][1] + w] / sfsum
                            locval += sfval * df
