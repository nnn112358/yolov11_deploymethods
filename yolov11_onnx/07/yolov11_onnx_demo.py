...
def postprocess(outputs, image_h, image_w):
    ...
    for index in range(head_num):
        output_cls = outputs[index * 2 + 0]
        output_reg = outputs[index * 2 + 1]
        for h in range(map_size[index][0]):
            for w in range(map_size[index][1]):
                ...
                for c in range(class_num):
                    val = output_cls[c * map_size[index][0] * map_size[index][1] + h * map_size[index][1] + w]
                    if val > cls_vlaue:
                        cls_vlaue = val
                        cls_index = c
                ...
                if cls_vlaue > object_thresh:
                    ...
                    for lc in range(4):
                        ...
                        locval = 0
                        for df in range(16):
                            temp = exp(output_reg[((lc * 16) + df) * map_size[index][0] * map_size[index][1] + h * map_size[index][1] + w])
                            output_reg[((lc * 16) + df) * map_size[index][0] * map_size[index][1] + h * map_size[index][1] + w] = temp
                            sfsum += temp

                        for df in range(16):
                            sfval = output_reg[((lc * 16) + df) * map_size[index][0] * map_size[index][1] + h * map_size[index][1] + w] / sfsum
                            locval += sfval * df
...
def detect(img_path):
    ...
    ort_session = ort.InferenceSession('./yolov11n.onnx')
