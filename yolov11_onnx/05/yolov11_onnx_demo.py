...
def postprocess(outputs, image_h, image_w):
    ...
    for i in range(0, anchors):
        ...
        for cl in range(64, class_num + 64, 1):
            val = output[i + cl * anchors]
            if val > cls_vlaue:
                cls_vlaue = val
                cls_index = cl
        ...
        if cls_vlaue > object_thresh:
            ...
            for lc in range(4):
                ...
                locval = 0
                for df in range(16):
                    temp = exp(output[(lc * 16 + df) * anchors + i])
                    output[(lc * 16  + df) * anchors + i] = temp
                    sfsum += temp

                for df in range(16):
                    sfval = output[(lc * 16 + df) * anchors + i] / sfsum
                    locval += (sfval * df)
