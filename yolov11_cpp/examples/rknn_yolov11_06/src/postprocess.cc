...
int GetResultRectyolov11::GetConvDetectionResult(int8_t **outputs, std::vector<int> &qnt_zp, std::vector<float> &qnt_scale, std::vector<float> &DetectiontRects)
{
    ...
    for (int index = 0; index < headNum; index++)
    {
        int8_t *output = outputs[index];
        quant_zp = qnt_zp[index];
        quant_scale = qnt_scale[index];
        ...
        for (int h = 0; h < mapSize[index][0]; h++)
        {
            for (int w = 0; w < mapSize[index][1]; w++)
            {
                ...
                for(int cl = 64; cl < class_num + 64; cl ++)
                {
                ...
                cls_vlaue = DeQnt2F32(cls_vlaue, quant_zp, quant_scale);
                ...
                if (cls_vlaue > object_thresh)
                {
                    ...
                    for (int lc = 0; lc < 4; lc++)
                    {
                        ...
                        {
                            locvaltemp = exp(DeQnt2F32(output[((lc * 16) + df) * mapSize[index][0] * mapSize[index][1] + h * mapSize[index][1] + w], quant_zp, quant_scale));
